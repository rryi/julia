...
function write_prompt(terminal, p::Prompt, hascolor::Bool)
    prefix = prompt_string(p.prompt_prefix)
    suffix = prompt_string(p.prompt_suffix)
    write(terminal, prefix)
    hascolor && write(terminal, Base.text_colors[:bold])
    width = write_prompt(terminal, p.prompt, hascolor)
    hascolor && write(terminal, Base.text_colors[:normal])
    write(terminal, suffix)
    return width
end

# returns the width of the written prompt
function write_prompt(terminal, s::Union{AbstractString,Function}, hascolor::Bool)
...
write_prompt(terminal, s::PrefixSearchState, hascolor::Bool) = write_prompt(terminal, s.histprompt.parent_prompt, hascolor)
